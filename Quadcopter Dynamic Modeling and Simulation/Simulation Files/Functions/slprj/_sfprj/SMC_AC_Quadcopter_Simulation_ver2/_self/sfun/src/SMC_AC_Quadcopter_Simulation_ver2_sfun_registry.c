...
{
  extern unsigned int
    sf_SMC_AC_Quadcopter_Simulation_ver2_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_SMC_AC_Quadcopter_Simulation_ver2_process_check_sum_call(nlhs,plhs,nrhs,
       prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_SMC_AC_Quadcopter_Simulation_ver2_autoinheritance_info(
  ...
  if (strcmp(machineName, "SMC_AC_Quadcopter_Simulation_ver2") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_SMC_AC_Quadcopter_Simulation_ver2_autoinheritance_info(nlhs,plhs,3,
      newRhs);
  }
...
{
  extern unsigned int sf_SMC_AC_Quadcopter_Simulation_ver2_third_party_uses_info
    ( int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 4) {
  ...
  if (strcmp(machineName, "SMC_AC_Quadcopter_Simulation_ver2") == 0) {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_SMC_AC_Quadcopter_Simulation_ver2_third_party_uses_info(nlhs,plhs,
      3,newRhs);
  }
...
{
  extern unsigned int
    sf_SMC_AC_Quadcopter_Simulation_ver2_updateBuildInfo_args_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 4) {
  ...
  if (strcmp(machineName, "SMC_AC_Quadcopter_Simulation_ver2") == 0) {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_SMC_AC_Quadcopter_Simulation_ver2_updateBuildInfo_args_info(nlhs,
      plhs,3,newRhs);
  }
...
{
  extern unsigned int
    sf_SMC_AC_Quadcopter_Simulation_ver2_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "SMC_AC_Quadcopter_Simulation_ver2") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_SMC_AC_Quadcopter_Simulation_ver2_get_eml_resolved_functions_info
      (nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_SMC_AC_Quadcopter_Simulation_ver2_method_dispatcher
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"SMC_AC_Quadcopter_Simulation_ver2")) {
    return(sf_SMC_AC_Quadcopter_Simulation_ver2_method_dispatcher(simstructPtr,
...
extern void SMC_AC_Quadcopter_Simulation_ver2_terminator(void);
void sf_machine_global_terminator(void)
{
  sfGlobalMdlStartCallCounts--;
  if (sfGlobalMdlStartCallCounts == 0) {
    SMC_AC_Quadcopter_Simulation_ver2_terminator();
...
extern void SMC_AC_Quadcopter_Simulation_ver2_initializer(void);
extern void SMC_AC_Quadcopter_Simulation_ver2_register_exported_symbols
  (SimStruct* S);
extern void SMC_AC_Quadcopter_Simulation_ver2_debug_initialize(struct
  SfDebugInstanceStruct*);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  SMC_AC_Quadcopter_Simulation_ver2_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfGlobalMdlStartCallCounts == 1) {
    ...
    if (!simModeIsRTWGen) {
      SMC_AC_Quadcopter_Simulation_ver2_debug_initialize
        (sfGlobalDebugInstanceStruct);
    }

    SMC_AC_Quadcopter_Simulation_ver2_initializer();
  }
